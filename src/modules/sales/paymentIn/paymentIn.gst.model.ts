...
// Main Invoice Schema
const paymentInGstSchema = new Schema<IPaymentIn>({
    paymentInType: {
        type: String,
...
paymentInGstSchema.index({ paymentInType: 1, gstType: 1 });

// Virtual for formatted dates (optional)
paymentInGstSchema.virtual('formattedPaymentInDate').get(function () {
    return this.paymentInDate ? new Date(this.paymentInDate).toLocaleDateString() : '';
});

paymentInGstSchema.virtual('formattedDueDate').get(function () {
    return this.dueDate ? new Date(this.dueDate).toLocaleDateString() : '';
});

// Virtual for payment status
paymentInGstSchema.virtual('paymentStatus').get(function () {
...
paymentInGstSchema.pre('save', function (next) {
...
const PaymentInGst = mongoose.model<IPaymentIn>('PaymentInGst', paymentInGstSchema);

export default PaymentInGst;