...
const debitNoteBaseSchema = z.object({
  debitNoteType: z.enum(['PURCHASE', 'PURCHASE_RETURN', 'DEBIT_NOTE', 'PAYMENT_OUT']).default('DEBIT_NOTE'),
  ...
  debitNoteDate: z.union([z.string(), z.date()]).default(() => new Date().toISOString()),
...
export const debitNoteSchema = debitNoteBaseSchema
.refine((data) => {
