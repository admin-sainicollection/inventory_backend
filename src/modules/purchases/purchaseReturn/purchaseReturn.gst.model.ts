...
// Main Invoice Schema
const purchaseReturnGstSchema = new Schema<IPurchaseReturn>({
    purchaseReturnType: {
        type: String,
...
// purchaseReturnGstSchema.index({ purchaseReturnNumber: 1 });
purchaseReturnGstSchema.index({ party: 1 });
purchaseReturnGstSchema.index({ purchaseReturnDate: 1 });
purchaseReturnGstSchema.index({ createdAt: 1 });
purchaseReturnGstSchema.index({ purchaseReturnType: 1, gstType: 1 });

// Virtual for formatted dates (optional)
purchaseReturnGstSchema.virtual('formattedPurchaseReturnDate').get(function () {
    return this.purchaseReturnDate ? new Date(this.purchaseReturnDate).toLocaleDateString() : '';
});

purchaseReturnGstSchema.virtual('formattedDueDate').get(function () {
    return this.dueDate ? new Date(this.dueDate).toLocaleDateString() : '';
});

// Virtual for payment status
purchaseReturnGstSchema.virtual('paymentStatus').get(function () {
...
purchaseReturnGstSchema.pre('save', function (next) {
...
const PurchaseReturnGst = mongoose.model<IPurchaseReturn>('PurchaseReturnGst', purchaseReturnGstSchema);

export default PurchaseReturnGst;