...
export const createEnquiryController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const enquiry = await createEnquiry(enquiryData, req.files as Express.Multer.File[]);
...
export const updateEnquiryController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const updatedEnquiry = await updateEnquiry(id as string, updateData, userId, req.files as Express.Multer.File[]);
